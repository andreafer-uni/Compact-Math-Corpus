...
def generate_full_stats(doc, output_stats_path, corpus_name="lahefferon-pdf", num_pages=525):
    ...
    dep_stats = {
        dep: dict(dep_stats[dep].most_common(50)) for dep in dep_counts
    }
    ...
    stats = {
        ...
        "pos_stats": {pos: {word: count for word, count in words} for pos, words in pos_stats.items()},
        "tag_stats": {tag: {word: count for word, count in words} for tag, words in tag_stats.items()},
        "dep_stats": dep_stats
    }
    ...
    end_time = time.time()
    print(f"Statistics saved in: {output_stats_path}")
