...
def extract_tfidf_before_conll(tsv_file, output_file):
    """
    Compute TF-IDF for ADJ-NOUN and NOUN-NOUN bigrams,
    treating each sentence as a document.
    The input must be a TSV file ('cmcorpus-spacyfeatures.tsv') 
    in spaCy features format, not a CoNLL file.
    """
    ...
    with open(output_file, "w", encoding="utf-8") as outfile:
        ...
        for bigram, tfidf in sorted(bigram_tfidf.items(), key=lambda x: x[1], reverse=True):
